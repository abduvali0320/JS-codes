let tbody = document.querySelector('tbody'),
  ...
  inputValue = {},
  _id = null
const modal = new bootstrap.Modal('#modal_depart', {
  keyboard: false
})
const handleSend = (e) => {
  e.preventDefault();
  ...
  if (_id) {
    axios.put(`${url}/department`, { ...inputValue, _id }, header)
      .then((res) => {
    ...
    axios.post(`${url}/department`, inputValue, header)
      .then((res) => {
        data = [res.data, ...data,]
        render()
        e.target.reset()
...
const handleDel = (_id) => {
  if (confirm("o'chirasimi ?")) {
    axios.delete(`${url}/department/${_id}`, header)
      .then((res) => {
...
const handleEdit = (id) => {
  ...
  axios.get(`${url}/department/${id}`, header)
    .then(res => {
...
const handleStatus = (_id) => {
  axios.get(`${url}/department/change/${_id}`, header)
    .then((res) => {
      data = data.map((c) => c._id === res.data._id ? res.data : c)
      render()
    })
