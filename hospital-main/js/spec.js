...
const handleSend = (e) => {
  ...
  if (_id) {
    axios.put(`${url}/spec`, { ...inputValue, _id }, header)
      .then((res) => {
    ...
    axios.post(`${url}/spec`, inputValue, header)
      .then((res) => {
        data = [res.data, ...data,]
        render()
        e.target.reset()
...
const get_api_data = () => {
  axios.get(`${url}/spec`, header)
    .then((res) => {
      data = res.data
      render()
    })
    .catch((err) => console.log(err))
    .finally(() => console.log('get mehtod spec'))
}
get_api_data()
const handleDel = (_id) => {
  if (confirm("o'chirasimi ?")) {
    axios.delete(`${url}/spec/${_id}`, header)
      .then((res) => {
...
const handleStatus = (_id) => {
  axios.get(`${url}/spec/change/${_id}`, header)
    .then((res) => {
      data = data.map((c) => c._id === res.data._id ? res.data : c)
      render()
    })
